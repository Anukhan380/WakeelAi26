...
// Uses OpenRouter's OpenAI-compatible endpoint. 'openrouter/free' is a
// built-in router that auto-picks a working free model for us, so we
// don't have to hardcode one specific model name that could later
// get deprecated or rate-limited.
// raw:true returns the full parsed response instead of just the text,
// used by the debug-ai route below.
function callAI(system, messages, raw) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://wakeel-ai26.vercel.app',
        'X-Title': 'WakeelAI',
  ...
  try {
    ...
    if (route === 'test') {
      res.json({
        ...
        openrouter: OPENROUTER_KEY ? '✅ Set' : '❌ Missing',
    ...
    // TEMP DEBUG: returns Gemini's raw response so we can see finish_reason etc.
    // Remove this route once the empty-response issue is fixed.
    if (route === 'debug-ai') {
      const raw = await callAI('You are a helpful assistant.', [{ role: 'user', content: 'Say hello in one sentence.' }], true);
      res.json(raw);
      return;
    }
